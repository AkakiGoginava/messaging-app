...
function Block-Action {
    ...
    [Console]::Error.WriteLine("Implementer guard blocked this action: $Message")
    exit 2
}
...
if ($toolName -in @("Edit", "Write")) {
    ...
    foreach ($pattern in $protectedPatterns) {
        if ($normalizedRelativePath -match $pattern) {
            Block-Action "the protected path '$normalizedRelativePath' is outside the Implementer role"
        }
    }
...
if ($toolName -in @("Bash", "PowerShell")) {
    ...
    foreach ($pattern in $blockedCommandPatterns) {
        if ($command -match $pattern) {
            Block-Action "the shell command crosses the Implementer role boundary"
        }
    }
